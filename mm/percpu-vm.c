			       ...
			       unsigned long vm_flags)
{
	...
	/*
	 * Warn and ignore if VM_SENSITIVE is specified, as sensitive
	 * memory allocation isn't supported for percpu allocator yet.
	 */
	WARN_ON_ONCE(vm_flags & VM_SENSITIVE);
	asi_unmap(ASI_GLOBAL_NONSENSITIVE, (void *)addr, size, /* sleepable = */true);
			    ...
			    int nr_pages, unsigned long vm_flags)
{
	...
	/*
	 * Warn and ignore when VM_SENSITIVE is specified, as sensitive
	 * memory allocation isn't supported for percpu allocator yet.
	 */
	WARN_ON_ONCE(vm_flags & VM_SENSITIVE);
	/*
